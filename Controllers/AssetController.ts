...
import { BonkleBuck, Option, OptionMap, OptionValue, Stock, SupplyDetails, toEmbed, Token, TokenContract, Trade, Transaction } from "../models"
...
export class AssetController {
    ...
    getCirculatingSupply(): MessageEmbed[] {
        let circulatingSupply: number = 0;
        let users = Object.keys(this.currentBalances);
        users.splice(users.indexOf('DEAD_WALLET'), 1);
        users.splice(users.indexOf('Bonkle Buck Broker'), 1);
        users.splice(users.indexOf('COCKSINO'), 1);
        users.splice(users.indexOf('BLOCK_REWARD'), 1)
        users.forEach(user => {
            console.log(`${user} has ${this.currentBalances[user]}`)
            circulatingSupply += this.currentBalances[user]
        })

        return [new MessageEmbed().setTitle('Supply Stats').addFields([
            {name: 'Total Circulating Supply', value: `${circulatingSupply}`},
            {name: 'Total mined', value: `${-this.currentBalances['BLOCK_REWARD']}`},
            {name: 'Ammount in Dead Wallets', value: `${-this.currentBalances['DEAD_WALLET']}`},
            {name: 'Ammount generated by Cocksino', value: `${-this.currentBalances['COCKSINO']}`},
            {name: 'Ammount generated by Stocks and Options', value: `${-this.currentBalances['Bonkle Buck Broker']}`}
        ])]

    }

    getTokenPortfolioEmbed(id: string): MessageEmbed {
        let tokens = Object.keys(this.currentTokens)
        let embed: MessageEmbed = new MessageEmbed().setTitle('Tokens')
        tokens.forEach(token => {
            if(this.currentTokens[token].distribution[id]) embed.addField(token, `${this.currentTokens[token].distribution[id]}`) 
        });
        return embed
    }
